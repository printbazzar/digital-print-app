...
function seed() {
  ...
  const dbData = {
    ...
    machines: [{ ...INITIAL_MACHINE, createdAt: now, updatedAt: now }],
    rates: INITIAL_RATES.map((r) => ({ ...r, createdAt: now, updatedAt: now })),
    media: INITIAL_MEDIA.map((m) => ({ ...m, createdAt: now, updatedAt: now })),
    inventoryMovements: [],
    wastageReasons: INITIAL_WASTAGE_REASONS.map((wr) => ({ ...wr, createdAt: now })),
    jobs: [],
    dailyCounters: [],
    notifications: [],
    auditLogs: [],
  ...
  console.log('✅ Database seeded successfully with Konica Minolta C3070 (Starting Counter: 1,067,426).');
}
